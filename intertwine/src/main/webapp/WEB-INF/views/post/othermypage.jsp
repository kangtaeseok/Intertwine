...
<div class="container" style="margin: 0 auto; padding-left: 250px; width: 1000px;">
    <div class="top" style="margin-top: 30px; display: flex;">
        <div class="profile" style="padding-left: 10px;">
            <div class="pic" style="width: 200px; height: 200px; border-radius: 50%; border: 0px solid black; overflow: hidden;">
                <a href="${pageContext.servletContext.contextPath}/page.do?friendId=${otheruser.userId}"><img src="resources/profile/default.png" style="width: 100%; height: 100%; object-fit: cover;"></a>
            </div>
        </div>
        <div class="profile-info" style="display: flex;">
            ...
            <div class="left" style="display: flex;">
                <div class="personal">
                    <ul>
                        <br>
                        <li style="list-style-type: none;"><h3>${ otheruser.userId } &nbsp;&nbsp;<a href="#"><i class="fa-solid fa-gear" style="color:black;"></i></a></h3></li>
    ...
    <div class="dummy2" style="height: 50px;"></div>
    <c:choose>     
        <c:when test="${isFollower > 0 && isFollowing > 0}">
        <p>당신이 팔로잉 하고 있습니다</p>
            <%-- <a class="follow" href="${pageContext.request.contextPath}/unfollowing.do?userId=${  }">언팔로우</a> --%>
           <form action="unfollowing.do" method="post">
            <input type="hidden" name="userId"
               value="${sessionScope.loginUser.userId}"> <input
               type="hidden" name="friendId" value="${FollowingId}">
            <button class="unfollow-btn"
               data-friend-id="${FollowingId}">언팔로우</button>
         </form> 
        </c:when>       
        <c:when test="${isFollowing > 0 && isFollower == 0}">
        ...
        <c:when test="${isFollower > 0 && isFollowing == 0}">
         <p>당신을 팔로잉 하고 있습니다</p>
           ...
           <form action="insertF.do" method="post">
            <input type="hidden" name="userId"
               value="${sessionScope.loginUser.userId}"> <input
               type="hidden" name="friendId" value="${FollowerId}">
                  <button type="submit" class="follow-btn" data-friend-id="${FollowerId}">팔로우</button>
                  </form>
        </c:when>
    ...
    <div class="middle" style="padding-top: 30px; display: flex;">
        ...
        <form action="search.do" style="border: none; background: transparent; padding: 5px; margin-right: 5px; cursor: pointer;">
           <input type="hidden" name="otherUserId" value="${ otheruser.userId }">
            <select class="search-condition" name="condition">
                <option value="all" selected>전체</option>
                <option value="tag">태그</option>
                <option value="content">콘텐츠</option>
            <input type="text" placeholder="검색할 키워드를 입력하세요" name="search" style="border: none; outline: none; background-color: #FFF; font-size: 13px;"> 
             <i class="fa fa-search" style="color: #000; font-size: 33px; margin-left: 5px; cursor: pointer; transition: transform 1s ease;"><input type="submit" style="style:none;"></i>
        </form>
        </div>
    ...
    <div class="bottom" style="display: flex;flex-wrap: wrap;width: 100%;" >
       ...
       <c:forEach var="gallery" items="${galleries}">
        ...
        <div class="options gallery-item" style="width: 320px; height:320px; margin-bottom: 30px;margin-right: 13px;position: relative" data-id="${ gallery.post.postId }">
            <input type="checkbox">
            ...
            <c:if test="${not empty gallery.video.videoURL and not empty gallery.image.imageURL}">
                <img src="resources/postimage/noimage.png" style="width: 100%;height: 100%;object-fit: contain;">
            </c:if>
